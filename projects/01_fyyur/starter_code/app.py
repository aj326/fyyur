...
# App Config.
# ----------------------------------------------------------------------------#
from starter_code.models import Venue, Artist, Show, app, db
...
def format_datetime(value, format='medium'):
    # https://stackoverflow.com/a/63295801
...
def venues():
    ...
    for loc in locations:
        venue_query = Venue.query.filter_by(city=loc[0], state=loc[1]).all()
        venue_list = []
        for venue in venue_query:
            # print(len(venue.shows))
            venue_list.append({'id': venue.id, 'name': venue.name, 'num_upcoming_shows': len(venue.shows)})
        result.append({'city': loc[0], 'state': loc[1], 'venues': venue_list})
    print(result)
    # print(cities)
    # print(venues)
    data = [{
...
def show_venue(venue_id):
    ...
    past_shows = []
    future_and_present_shows = []
    for show in venue.shows:
        date = show.start_time
        if isinstance(show.start_time, str):
        ...
        if date < datetime.now():
            ...
            future_and_present_shows.append({
                "artist_id": show.artist_id,
                "artist_name": Artist.query.get(show.artist_id).name,
                "artist_image_link": Artist.query.get(show.artist_id).image_link,
                "start_time": show.start_time})
    ...
    # print(venue.genres)
    # print(len(venue.genres[1:-1].split(', ')))
    # print(venue.genres[1:-1].split(', '),type(venue.genres),)
    result = {
        "id": venue.id,
        "name": venue.name,
        ...
        "upcoming_shows": future_and_present_shows,
        "past_shows_count": len(past_shows),
        "upcoming_shows_count": len(future_and_present_shows),
    }
    # data1 = {
    #     "id": 1,
    #     "name": "The Musical Hop",
...
def create_venue_submission():
    ...
    try:
        ...
        # print(venue)
        jsoned = json.dumps(form.genres.data)
...
def shows():
    ...
    # DONE: replace with real venues data.
    result = []
    for show in Show.query.order_by('id').all():
        print(Venue.query.get(show.venue_id).name)
        result.append({'venue_id': show.venue_id,
                       'venue_name': Venue.query.get(show.venue_id).name,
                       'artist_id': show.artist_id,
                       'artist_name': Artist.query.get(show.artist_id).name,
                       'artist_image_link': Artist.query.get(show.artist_id).image_link,
